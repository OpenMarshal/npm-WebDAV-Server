...
module.exports = (test, options, index) => test('resource tester on the wrapped virtual resources', (isValid, server) =>
{
    ...
    new webdav.ResourceTester({
        ...
        (willCreate, cb) => cb(new webdav.ResourceWrapper(new webdav.VirtualFile('test')))
    ).run((results) => {
    ...
    new webdav.ResourceTester({
        ...
        (willCreate, cb) => cb(new webdav.ResourceWrapper(new webdav.VirtualFolder('test')))
    ).run((results) => {
        isValid(results.all.isValid, results.all.errors);
    })
})